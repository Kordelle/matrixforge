...
FACTORIES, CLIENT_CITIES, and FACTORY_INDEX are always loaded from this module.
CATALOG_ITEMS now live in python/data/catalog_seed.ndjson (generated by
python/generate_catalog.py) and are indexed into ChromaDB at container startup.
The CatalogItem TypedDict below defines the enterprise schema for both the
NDJSON seed file and the ChromaDB metadata payload.
"""
...
class CatalogItem(TypedDict):
    """Enterprise-grade SKU schema — mirrors the NDJSON seed file structure."""
    # Core identity
    sku: str
    name: str
    description: str           # Rich text — embedded by ChromaDB for semantic search

    # Taxonomy
    category: str              # e.g. "Panel Systems", "Seating", "Worksurfaces"
    subcategory: str           # e.g. "Panel Frames", "Task Seating"
    product_family: str        # e.g. "Compose", "Fern", "Zody II"

    # Supply chain
    origin: str                # Factory id: FAC_A..FAC_E
    cost: float                # USD per unit (base, ex-freight)
    carbon_score: int          # 0-100, lower is greener
    weight_kg: float           # Gross weight — drives LTL freight calculation

    # Sustainability & compliance
    certifications: list[str]  # e.g. ["BIFMA", "GREENGUARD Gold", "FSC"]
    recycled_content_pct: float
    epd_certified: bool        # Environmental Product Declaration
    end_of_life: str           # "Recyclable" | "Take-back program" | "Landfill"
    hts_code: str              # Harmonized Tariff Schedule — international customs
    country_of_origin: str

    # Commercial
    lead_time_override: int | None  # Days; None = derived from factory capacity
    warranty_years: int
    finish_options: int             # Number of available finish/colour configurations

    # Suitability
    environments: list[str]         # ["corporate", "healthcare", "education", ...]
    compatible_families: list[str]  # Compatible product family names
...
# ---------------------------------------------------------------------------
# NOTE: CATALOG_ITEMS has moved to python/data/catalog_seed.ndjson
# Run python/generate_catalog.py to regenerate the ~30k SKU seed file.
# The ChromaCatalogStore in python/vector_store.py loads and indexes it
# at container startup.  A lightweight in-memory fallback is maintained
# in vector_store.py for use while background indexing is in progress.
# ---------------------------------------------------------------------------

# Legacy slim catalog kept only as a dev fallback (used when NDJSON is absent)
_FALLBACK_CATALOG: list[CatalogItem] = [
